...
// Initialize the Pezzo OpenAI client
export const openai = new PezzoOpenAI(pezzo, configuration);
```

<Tip>
  The `PezzoOpenAI` class wraps the native `OpenAI` class. This ensures a seamless experience, and allows you to use all OpenAI features.
</Tip>
...
<Tabs>
  ...
  <Tab title="Without Prompt Management">
    ```ts
    const response = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      temperature: 0,
