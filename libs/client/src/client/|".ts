...
export class PezzoOpenAIApi extends OpenAIApi {
  ...
  override async createChatCompletion(
    ...
    const response = super.createChatCompletion({
      model: "gpt-3.5-turbo",
      messages: [{
        role: "user",
        content: "hi my name Borat"
      }],
    }).then(result => {
    ...
    try {
      ...
      // console.log("originalArgs", JSON.stringify(originalArgs, null, 2));
      console.log("args", args);

      
      // const result = await super.createChatCompletion.call(this, {
      //   model: "gpt-3.5-turbo",
      //   messages: [
      //     {
      //       role: "user",
